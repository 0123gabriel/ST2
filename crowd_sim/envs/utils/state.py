class FullState(object):
    def __init__(self, px, py, vx, vy, radius, gx, gy, v_pref, theta, min_dist):
        self.px = px
        self.py = py
        self.vx = vx
        self.vy = vy
    ...
    def __add__(self, other):
        return other + (self.px, self.py, self.vx, self.vy, self.radius, self.gx, self.gy, self.v_pref, self.theta, self.min_dist)

    def __str__(self):
        return ' '.join([str(x) for x in [self.px, self.py, self.vx, self.vy, self.radius, self.gx, self.gy,
                                          self.v_pref, self.theta, self.min_dist]])
